      ...
      textNode.on('dblclick', () => {
        ...
        textarea.addEventListener('keydown', function (e) {
          ...
          if (e.keyCode === 13 |e.keyCode === 9) {
            ...
            if (textarea.value != "Add Label Here"){
...
display_element.querySelector('#jspsych-draw-circles-done-btn').addEventListener('click', function(){
    ...
    if (no_label_counter > 0){
        alert("Please label all circles!")
    }else if (circles.length === 0){
        alert("You haven't drawn any circles!")
        
    }else{
            
      for(var i=0; i<circles.length; i++){
        
        current_circle = circles[i] // currently assuming the labels are corresponding

...
df.corpus_filler <- read.csv("../../data/corpus/corpusEMV_filler.csv") %>%
  ...
  select(!(contains("cue_frequency") | contains("word1_frequency") | contains("word2_frequency"))) %>% # ignore base frequencies for now
  mutate(cue_id = cue_E)
```

```{r corpus - frequency, include = F}
#find triads_omit, used whenever analysis uses raw collocates 
triads_omit <- df.corpus_filler %>% 
  filter((word1_match_frequency_E == 0 & word2_match_frequency_E == 0) |
          #(word1_match_frequency_M == 0 & word2_match_frequency_M == 0) |
           (word1_match_frequency_V == 0 & word2_match_frequency_V == 0)) %>%
  pull(cue_id)
```
...
```{r getting triads from the 3 languages}
...
df <- df %>%
  mutate(word1_match_frequency_prop_E = 
         ...
         word1_match_cosine_prop_E = 
           word1_match_cosine_E / (word1_match_cosine_E + word2_match_cosine_E),
         word1_match_cosine_prop_M = 
           word1_match_cosine_M / (word1_match_cosine_M + word2_match_cosine_M),
         word1_match_cosine_prop_V = 
           word1_match_cosine_V / (word1_match_cosine_V + word2_match_cosine_V), 
         )

write.csv(df, "../../data/data_filler_USCNVN_ENZHVI.csv", row.names = F)
write.csv(triads_omit, "../../data/fillers_omit.csv", row.names = F)
```
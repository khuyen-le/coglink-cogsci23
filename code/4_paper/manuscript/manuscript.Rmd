...
#df <- read.csv("../../../data/data_USCNVN_ENZHVI_0623.csv")
...
```{r calculate range of beta statistics, warning=F, include=F}
beta.single_corpus_freq <- c(
  round(fixef(fit.EN_US)["theme_freq_prop_E"], 2),
  round(fixef(fit.VI_US)["theme_freq_prop_V"], 2), 
  round(fixef(fit.ZH_US)["theme_freq_prop_M"], 2), 
  
  round(fixef(fit.EN_CN)["theme_freq_prop_E"], 2),
  round(fixef(fit.VI_CN)["theme_freq_prop_V"], 2), 
  round(fixef(fit.ZH_CN)["theme_freq_prop_M"], 2), 
  
  round(fixef(fit.EN_VN)["theme_freq_prop_E"], 2),
  round(fixef(fit.VI_VN)["theme_freq_prop_V"], 2), 
  round(fixef(fit.ZH_VN)["theme_freq_prop_M"], 2)
  
)
```
We found that all corpora are significant predictors of all cultural context responding, with p < 0.05 and $\beta$ from `r min(beta.single_corpus_freq)` - `r max(beta.single_corpus_freq)`. (For a full report, see Appendix.)
...
Similar to the results from the analysis with raw lexical co-occurrences, we found that all corpora are significant predictors of all cultural context responding, with p < 0.05 and $\beta$ from `r min(beta.single_corpus_cos)` - `r max(beta.single_corpus_cos)`. (For a full report, see Appendix.)
...
plot.coeffs_freq <- ggplot(data = df.USVNCN_coeffs_freq, 
...
We test whether using the same approach, we can also predict filler similarity judgements. 
...
``` 
No effect of country on filler responding (Figure 4).

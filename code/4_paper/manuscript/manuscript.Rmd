...
## Procedure
\begin{CJK*}{UTF8}{gbsn}
Each participant completed all 105 triads in blocks of 21 trials at a time (10 test triads, 10 filler triads, and 1 attention check per page), by selecting the match most related to the cue. We elicited similarity match responses by asking "Which thing is most closely related to the bolded [cue] item?" In Mandarin Chinese, we used the phrasing for "most closely related" reported by Ji et al. (2004): "哪个事物与加粗项关联最紧密?" And in Vietnamese, we translated this as "Thứ nào liên quan nhất với thứ được in đậm?".
...
In sum, we replicated the differences documented by @Ji2004 between the US and China. However, we did not find that Vietnamese participants grouped particularly with Chinese participants. Instead they were intermediate between the two. 
...
<!-- To do this,  -->
we use a mixed-effects logistic regression fit individually for each country. Our models predicted responses by individual participants to particular triad (0=taxonomic or 1=thematic) with fastText predictions (proportion of cosine distance) as a fixed effect and participant and triad as random effects.
...
[^5]: With the preregistered exclusion criteria, the English (EN) and Mandarin (ZH) corpora were significant predictors.

For Vietnamese participants' responses, only the Vietnamese (VI) and Mandarin (ZH) corpora were significant predictors[^6].
...
[^6]: With the preregistered exclusion criteria, all three corpora were significant predictors.
...
@Ji2004 established that culture-aligned differences in this paradigm exist, even when the test language is held constant, concluding that "it is culture (independent of the testing language) that led to different grouping styles" in their study. Our data provide a cautionary note to this conclusion, suggesting that while cultural differences in similarity judgments exist, we might get better traction on modelling cross-cultural variation through environmental statistics such as lexical co-occurrence. 

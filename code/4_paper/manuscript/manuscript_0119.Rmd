...
```{r echo=FALSE, warning=FALSE, cache = TRUE, fig.env="figure", fig.pos = "H", fig.align = "center", fig.width=2, fig.height=2, fig.cap="Proportion of thematic responses by country."}
...
<!-- ```{r calculate range of beta statistics for lexical cooc, warning=F, include=F} -->
...
<!-- ) -->
<!-- ``` -->
<!-- We found that all corpora are significant predictors of all cultural context responding, with p < 0.05 and $\beta$ from `r # min(beta.single_corpus_freq)` to `r #max(beta.single_corpus_freq)`. (For a full report, see Appendix.) -->
...
```{r warning=FALSE, include = F, cache = TRUE, fig.env="figure", fig.width=2, fig.height=2, paged.print=TRUE}
# df.country_sum <- df.country %>%
#   group_by(country) %>%
#   summarize(mean_theme_resp_percent = mean(theme_resp_percent), 
#             sd_theme_resp_percent = sd(theme_resp_percent))
...
Using the same model structure as Analysis 2 ()
<!-- #### Raw co-occurrences -->
...
print(tab1, type="latex", comment = F, table.placement = "t", include.rownames = FALSE)
```

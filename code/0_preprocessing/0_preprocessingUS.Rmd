...
#folder_path = "~/Triad/data_expt/data_E/"
...
df <- df_raw %>%
  mutate(responses = ifelse(responses == "hammer ", "hammer", responses)) %>%
  mutate(top_opt = ifelse(top_opt == "hammer ", "hammer", top_opt)) %>%
  mutate(bottom_opt = ifelse(bottom_opt == "hammer ", "hammer", bottom_opt))

write.csv(df, "data_E_0319.csv")

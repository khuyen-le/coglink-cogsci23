...
#folder_path = "~/Triad/data_expt/data_M/"
...
#write.csv(df, "data_anon0115_full.csv")
...
df <- df_raw %>%
  mutate(responses = ifelse(responses == "锤子 ", "锤子", responses)) %>%
  mutate(top_opt = ifelse(top_opt == "锤子 ", "锤子", top_opt)) %>%
  mutate(bottom_opt = ifelse(bottom_opt == "锤子 ", "锤子", bottom_opt))

write.csv(df, "data_M_0530.csv")

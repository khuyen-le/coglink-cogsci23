...
```
# Does adding country help with the model (compared to with just English co-occurrence)? 
```{r}
fit.language = glmer(responses_theme ~ theme_freq_prop_E + (1 | subject) + (1 | triad), 
          data = df %>% filter(!(near(theme_frequency_E, .Machine$double.eps) & 
                                  near(tax_frequency_E, .Machine$double.eps))),  
...
fit.country_language = glmer(responses_theme ~ theme_freq_prop_E * country + (1 | subject) + (1 | triad), 
          data = df %>% filter(!(near(theme_frequency_E, .Machine$double.eps) & 
                                  near(tax_frequency_E, .Machine$double.eps))),  
...
Adding country improves the model (thematic proportion of lexical co-occurrence, country, and their interaction were all significant predictors).
